...
class DetailReportModal(private val context: Context) : Dialog(context) {
    ...
    private fun setLayout() {
        ...
        titleBar.tvTitleBar.text =
            if (withName) if (isCourier) data.nama_gudang else data.nama else "Detail Laporan"
        ...
        binding.tvDistance.text =
            " ${if (data.is_approved == "1") "Approved" else "Menunggu"} | ${data.distance_visit} km dari titik"
        binding.date.text = data.date_visit
        setDateReport()
        binding.description.text = data.laporan_visit
        setReportType()
        binding.approveMessage.text =
            data.approve_message.let { if (!it.isNullOrEmpty()) it else "Tidak ada feedback" }
    ...
    private fun setDateReport() {
        val date =
            SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).parse(data.date_visit)
    ...
    private fun openChecklistReport() {
        val userFullName =
            if (!contactName.isNullOrEmpty()) if (iUserID.isNullOrEmpty()) "saya" else "$userFullName"
