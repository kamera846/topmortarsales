...
import com.topmortar.topmortarsales.commons.RENVI_JATEM2
        ...
        lifecycleScope.launch {
            try {
                ...
                val response = when (userKind) {
                    USER_KIND_ADMIN -> {
                        if (selectedCity != null) apiService.jatemPenagihanFilter(dst = userDistributorId, idCity = selectedCity?.id!!, type = "jatem2")
                        else apiService.jatemPenagihan(dst = userDistributorId, type = "jatem2")
                    } else -> apiService.jatemPenagihanFilter(dst = userDistributorId, idCity = userCity, type = "jatem2")
                }
            ...
            rvAdapter = RencanaVisitRVA(listItem, object: RencanaVisitRVA.ItemClickListener {
                override fun onItemClick(data: RencanaVisitModel?) {
                    ...
                    context?.let { ctx ->
                        ...
                        data?.let { contactData ->
                            intent.putExtra(ACTIVITY_REQUEST_CODE, MAIN_ACTIVITY_REQUEST_CODE)
                            intent.putExtra(CONST_CONTACT_ID, contactData.id_contact)
                            intent.putExtra(CONST_NAME, contactData.nama)
                            intent.putExtra(CONST_PHONE, contactData.nomorhp)
                            intent.putExtra(CONST_BIRTHDAY, contactData.tgl_lahir)
                            intent.putExtra(CONST_OWNER, contactData.store_owner)
                            intent.putExtra(CONST_LOCATION, contactData.id_city)
                            intent.putExtra(CONST_MAPS, contactData.maps_url)
                            intent.putExtra(CONST_ADDRESS, contactData.address)
                            intent.putExtra(CONST_STATUS, contactData.store_status)
                            intent.putExtra(CONST_KTP, contactData.ktp_owner)
                            intent.putExtra(CONST_PAYMENT_METHOD, contactData.payment_method)
                            intent.putExtra(CONST_TERMIN, contactData.termin_payment)
                            intent.putExtra(CONST_PROMO, contactData.id_promo)
                            intent.putExtra(CONST_REPUTATION, contactData.reputation)
                            intent.putExtra(CONST_DATE, contactData.created_at_store)
                            intent.putExtra(CONST_WEEKLY_VISIT_STATUS, contactData.tagih_mingguan)
                            intent.putExtra(REPORT_SOURCE, PENAGIHAN_REPORT_RENVI)
                            intent.putExtra(RENVI_SOURCE, RENVI_JATEM2)
                            intent.putExtra(CONST_INVOICE_ID, contactData.id_invoice)
