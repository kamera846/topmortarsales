...
import com.topmortar.topmortarsales.commons.MG_REPORT_RENVI
import com.topmortar.topmortarsales.commons.RENVI_MG
...
import com.topmortar.topmortarsales.databinding.FragmentMGBinding
 ...
 */
class MGFragment : Fragment() {

    private var _binding: FragmentMGBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMGBinding.inflate(inflater, container, false)
        ...
        lifecycleScope.launch {
            try {
                ...
                val response = when (userKind) {
                    USER_KIND_ADMIN -> {
                        if (selectedCity != null) apiService.targetMg(idCity = selectedCity?.id!!)
                        else apiService.targetMgDst(idDistributor = userDistributorId)
                    } else -> apiService.targetMg(idCity = userCity)
                }
                ...
                when (response.status) {
                    ...
                    RESPONSE_STATUS_EMPTY -> {
                        ...
                        setRecyclerView(listItem)
                        loadingState(true, "Belum ada target untuk MG!")
                        showBadgeRefresh(false)
                ...
                override fun onItemClick(data: RencanaVisitModel?) {
                    context?.let { ctx ->
                        ...
                        if (data != null) {
                            ...
                            intent.putExtra(REPORT_SOURCE, MG_REPORT_RENVI)
                            intent.putExtra(RENVI_SOURCE, RENVI_MG)
