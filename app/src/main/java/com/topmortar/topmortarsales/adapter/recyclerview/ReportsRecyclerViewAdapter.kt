...
class ReportsRecyclerViewAdapter : RecyclerView.Adapter<ReportsRecyclerViewAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: ItemReportsBinding) :
        ...
        fun bind(item: ReportVisitModel, position: Int) {
            ...
            val date =
                SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).parse(item.date_visit)
            ...
            binding.date.text =
                if (withName == true) "$stringDistance km\n$dateFormat" else dateFormat

            val layoutParams = binding.root.layoutParams as ViewGroup.MarginLayoutParams
            if (layoutStatus == LAYOUT_GRID) {
                if (position == 0 || position == 1) layoutParams.topMargin =
                    convertDpToPx(16, context as Activity)
                if (position == listItem.size - 1 || position == listItem.size - 2) layoutParams.bottomMargin =
                    convertDpToPx(16, context as Activity)
            } else {
                if (position == 0) layoutParams.topMargin = convertDpToPx(16, context as Activity)
                if (position == listItem.size - 1) layoutParams.bottomMargin =
                    convertDpToPx(16, context as Activity)
            }
            ...
            if (item.has_checklist.isNotEmpty() && item.has_checklist == "1") binding.checklistIndicator.setImageResource(
                R.drawable.checklist_blue
            )
