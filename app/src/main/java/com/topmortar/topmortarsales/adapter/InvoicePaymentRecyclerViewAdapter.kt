...
class InvoicePaymentRecyclerViewAdapter: RecyclerView.Adapter<InvoicePaymentRecyclerViewAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: InvoicePaymentModel) {
            ...
            tvName.text = DateFormat.format(dateString = item.date_payment, inputFormat = "yyyy-MM-dd HH:mm:ss", outputFormat = "EEEE, dd MMMM yyyy")
...
//            tvDescription.text = item.debt.let { if (it != "0") "Remaining - ${CurrencyFormat.format(amount = it.toDouble())}" else "Paid"}
            tvDescription.text = DateFormat.format(dateString = item.date_payment, inputFormat = "yyyy-MM-dd HH:mm:ss", outputFormat = "HH:mm") + " WIB"
