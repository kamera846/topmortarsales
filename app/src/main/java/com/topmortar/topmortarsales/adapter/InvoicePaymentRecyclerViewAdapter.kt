...
class InvoicePaymentRecyclerViewAdapter :
    RecyclerView.Adapter<InvoicePaymentRecyclerViewAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: InvoicePaymentModel) {
            ...
            tvName.text = DateFormat.format(
                dateString = item.date_payment,
...
//            tvDescription.text = item.debt.let { if (it != "0") "Remaining - ${CurrencyFormat.format(amount = it.toDouble())}" else "Paid"}
            tvDescription.text = DateFormat.format(
                dateString = item.date_payment,
                inputFormat = "yyyy-MM-dd HH:mm:ss",
                outputFormat = "HH:mm"
            ) + " WIB"
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_invoice_payment, parent, false)
